...
{
    // Convert sling:// to sling:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sling://"))
    {
        uri.replace(0, 12, "sling:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Sling.lnk";
}
...
{
    return GetAutostartDir() / "sling.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Sling-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  sling-qt [" + tr("command-line options") + "]                     " + "\n";
