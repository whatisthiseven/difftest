...
bool RichListDB::Open(const char* pszMode)
{
    ...
    {
        LogPrintf("RichListDB::open() - DB does not exist.");
        return false;
    };
    ...
    if (!s.ok())
    {
        LogPrintf("RichListDB::open() - Error opening db: %s.", s.ToString().c_str());
        return false;
    };
...
bool RichListDB::TxnCommit()
{
    ...
    if (!status.ok())
    {
        LogPrintf("RichListDB batch commit failure: %s", status.ToString().c_str());
        return false;
    };
...
bool RichListDB::ReadRichList(CRichListData& richListData)
{
    ...
    if (readFromDb)
    {
        ...
        if (!s.ok())
        {
            if (s.IsNotFound())
                return false;
            LogPrintf("RichListDB read failure: %s", s.ToString().c_str());
            return false;
        };
    };
    ...
    try {
        ...
        LogPrintf("RichListDB::ReadRichList() unserialize threw: %s.", e.what());
        return false;
    }
...
bool RichListDB::WriteRichList(CRichListData richListData)
{
    ...
    if (!s.ok())
    {
        LogPrintf("RichListDB write failure: %s", s.ToString().c_str());
        return false;
    };
