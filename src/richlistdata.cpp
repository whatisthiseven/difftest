		      ...
		      if(!fInvalid)
		      {
			    ...
			    { 
			        ...
			        if(ExtractDestination(atxout.scriptPubKey, dest))
			        {
			    	        ...
			    	        std::string slingAddress = CBitcoinAddress(dest).ToString();
				        CRichListDataItem richListDataItem;
				        richListDataItem.sAddress = slingAddress;
				        if(richListData.mapRichList.find(slingAddress) == richListData.mapRichList.end())
				            richListData.mapRichList.insert(make_pair(slingAddress, richListDataItem));
					
				        richListData.mapRichList[slingAddress].nBalance = richListData.mapRichList[slingAddress].nBalance - atxout.nValue;
