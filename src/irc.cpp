...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #slingcoinTEST\r");
            Send(hSocket, "WHO #slingcoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #slingcoin\n");
            // randomly join #slingcoin00-#slingcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #slingcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #slingcoin%02d\r", channel_number).c_str());
        };
